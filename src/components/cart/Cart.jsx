...
const Cart = () => {
  ...
  const handleIncrement = (id, quality) => {
    if (quality < 99) {
      updateCartItem({ id, quality: quality + 1 });
    }
  };
  ...
  const invoiceSend = () => {
    ...
    axios
      .post("https://tsetsegtuw.templateapi.xyz/tsetsegtuv/invoice", {
        price: totalAmount,
      })
      .then((e) => {
        ...
        <div className="space-y-6">
          {cart.map((item) => (
            ...
            >
              ...
              <div className="flex items-center space-x-4">
                <img
                  src={`https://tsetsegtuw.templateapi.xyz/tsetsegtuv/${
              ...
              <div className="flex items-center space-x-4">
                <button
                  onClick={() => handleDecrement(item.id, item.quality)}
                ...
                <button
                  onClick={() => handleIncrement(item.id, item.quality)}
                  className="px-2 py-1 border border-gray-300 rounded-md hover:bg-gray-200"
                >
                  +
                </button>
              ...
              <div className="text-lg font-semibold">
                {Intl.NumberFormat("mn-MN").format(item.price * item.quality)}₮
              </div>
          ...
          <div className="flex justify-between items-center border-t border-gray-300 pt-4 mt-6">
            ...
            <span className="text-2xl font-semibold">
              {Intl.NumberFormat("mn-MN").format(totalAmount)}₮
            </span>
          </div>
        </div>
      ) : (
      ...
      {cart && cart.length > 0 && (
        <div className="mt-6">
          {isLoading ? (
            ...
            <button
              onClick={() => invoiceSend()}
              className="w-full px-10 bg-pink-500 text-white py-3 rounded-md text-lg font-semibold hover:bg-pink-600"
            >
              Үргэлжлүүлэх
            </button>
          )}
